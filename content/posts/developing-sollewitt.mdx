...
Fifteen wall drawings implemented so far. Each one required a different step handler in the engine.
...
| --------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `handleDivideAndLines`      | `divide` + `lines`     | Grid of cells with parallel line hatching. Supports `unique` (one direction per cell) and `progressive` (cumulative directions) modes. Optional colors per direction. | #11, #17, #56, #87 |
| `handleBands`               | `bands`                | Randomly placed bands of parallel lines in specified directions.                                                                                                      | #46, #47           |
| `handlePegboard`            | `pegboard`             | Grid of colored squares (closed paths that get filled).                                                                                                               | #38                |
| `handleWobblyLines`         | `wobbly-lines`         | Vertical lines with per-segment random displacement for hand-drawn effect.                                                                                            | #11                |
...
| `handleGridWobbly`          | `grid-wobbly`          | Grid of cells where each cell has wobbly lines in one random direction.                                                                                               | #88                |
