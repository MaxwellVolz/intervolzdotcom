...
export default function EarthScene() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      renderer.setAnimationLoop(() => {
        ...
        OOI.debugLine.geometry.setFromPoints([
          satPos,
        ...
        if (now - lastTelemetryTimeRef.current >= telemetryInterval) {
          ...
          while (
            telemetry.time.length > 0 &&
            now - telemetry.time[0] > maxDurationSeconds
          ) {
        ...
        const dir = new THREE.Vector3()
          .subVectors(new THREE.Vector3(0, 0, 0), satPos)
          .normalize();
        ...
        const azElQuat = new THREE.Quaternion().setFromEuler(
          new THREE.Euler(-el, az, 0, 'YXZ')
        ); // negative el to go *down* from nadir
      ...
      gui = new GUI();
      gui
        .add(debugSettings, 'chaseSatellite')
      ...
      gui
        .add(debugSettings, 'orbitRadius', 100, 600)
        .name('Orbit Radius (1000 km)')
        .onChange(updateOrbit);
      // gui.add(debugSettings, 'orbitInclination', -90, 90).name('Inclination (°)').onChange(updateOrbit);

      gui
        .add(debugSettings, 'coneAzimuthOffset', -180, 180)
        .name('Cone Azimuth (°)');
      gui
        .add(debugSettings, 'coneElevationOffset', -90, 90)
        .name('Cone Elevation (°)');
  ...
  return (
    <>
      <Seo title="Earth" path="/earth/" noindex />
      <div
        ref={mountRef}
